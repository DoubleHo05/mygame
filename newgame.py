...
class Game:
    def __init__(self):
        ...
        # the number of map, car and coin that user chose
        self.map = 0
        self.car = 0
        self.coin = 0
    ...
    def list_maps(self):       
        ...
        if self.chose_map == False:          
            ...
            for i in range(1, MAPS + 1):
                maps_image = pygame.transform.scale(pygame.image.load(f'map{i}.jpg').convert_alpha(), (150, 150))
        ...
        if self.map > 0 and self.chose_map == False:
            self.map_image = pygame.image.load(f'map{self.map}.jpg')
            self.chose_map = True
    ...
    def bet_money(self):
        ...
        if self.changed_name and self.chose_coin == False:
            ...
            for i in range(1, COINS + 1):                
                coins_image = pygame.transform.scale(pygame.image.load(f'coin{i}.png').convert_alpha(), (150, 150))
        ...
        if self.coin > 0 and self.chose_coin == False:
            ...
            print(f'So tien cuoc la {self.coin}')
